    ...
    switch($dest)
    {
        case 'I':
            ...
            if(ob_get_contents()) {
            	...
            	$this->Error('Some data has already been output, can\'t send PDF file - from html2pdf case I1');
            	var_dump($out1);
            }
           if(php_sapi_name()!='cli')
            {
              if(headers_sent($hdrfile, $hdrline))
                   $this->Error('Some data has already been output to browser, can\'t send PDF file - from html2pdf case I2 from '.$hdrfile.', '.$hdrline);
        ...
        case 'D':
            //Download file
            if(ob_get_contents())
                $this->Error('Some data has already been output, can\'t send PDF file - from html2pdf case D1');
            ...
            if(isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'],'MSIE'))
               ...
               if(headers_sent())
                  $this->Error('Some data has already been output to browser, can\'t send PDF file - from html2pdf case D2');
